...
export const productApi = {
  getAll: (filters?: ProductFilters) => kyClient.get<ProductResponse>('/products/'),
  getById: (id: number | string) => kyClient.get<ProductResponse>(`/products/${id}/`),
  create: (data: CreateProductData) => kyClient.post<ProductResponse>('/products/', data),
  update: (id: number | string, data: Partial<CreateProductData>) =>
    kyClient.patch<ProductResponse>(`/products/${id}/`, data),
  remove: (id: number | string) => kyClient.delete<void>(`/products/${id}/`),
};
```
...
- Only import from `@tanstack/react-query`, `ky`, toast library, error helper, and Layer 1 files.
...
- Always use `select` to unwrap the ky response before returning data.
...
| Unwrap ky data               | `select: (r) => r.data`                             |
